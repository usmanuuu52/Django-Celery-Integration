...
"""defines the parse tree components for Choco templates."""

from choco import errors, ast, util, filters, compat
import re
...
class Ui(Expression) :
    ...
    """defines an ui tag session.

    <%# hotpost(name) />

    """

    def __init__(self, text, ui_name, ui_args, **kwargs):
        super(Expression, self).__init__(**kwargs)
        self.text = text
        self.code = ast.PythonCode(text, **self.exception_kwargs)
