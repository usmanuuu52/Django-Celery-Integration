...
import time
import re
from choco.pygen import PythonPrinter
from choco import util, ast, nodes, filters, errors
...
def compile(node,
    ...
    """Generate module source code given a nodes node,
...
class _GenerateRenderMethod(object):
    ...
    def __init__(self, printer, compiler, node):
        ...
        self.in_def = isinstance(node, (nodes.DefTag, nodes.BlockTag))
    ...
    def write_toplevel(self):
        ...
        self.printer.writeline("from choco import runtime, filters, cache")
    ...
    def write_inherit(self, node):
        ...
        self.printer.writelines(
            "def _choco_inherit(template, context):",
            "_choco_generate_namespaces(context)",
    ...
    def write_namespaces(self, namespaces):
        ...
        self.printer.writelines(
            "def _choco_get_namespace(context, name):",
            "try:",
        ...
        self.printer.writeline("def _choco_generate_namespaces(context):")
    ...
    def write_variable_declares(self, identifiers, toplevel=False, limit=None):
        ...
        if toplevel and getattr(self.compiler, 'has_ns_imports', False):
            ...
            for ident, ns in self.compiler.namespaces.items():
                if 'import' in ns.attributes:
                    self.printer.writeline(
                        "_choco_get_namespace(context, %r)."
        ...
        for ident in to_write:
            if ident in comp_idents:
                ...
                self.printer.writeline(
                    "%s = _choco_get_namespace(context, %r)" %
                    (ident, ident)
                )
            else:
    ...
    def visitUi(self, node):
        self.printer.start_source(node.lineno)
        self.printer.writeline("runtime._include_ui(context, '%s', _template_uri, %s)" % (
            node.ui_name, node.ui_args))
    ...
    def visitControlLine(self, node):
        if node.isend:
            ...
            if self.compiler.enable_loop and node.keyword == 'for':
                text = mangle_choco_loop(node, self.printer)
            else:
                text = node.text
            ...
            if not children or (
                    compat.all(isinstance(c, (nodes.Comment,
                                              nodes.ControlLine))
                               for c in children) and
                    compat.all((node.is_ternary(c.keyword) or c.isend)
                               for c in children
                               if isinstance(c, nodes.ControlLine))):
                self.printer.writeline("pass")
...
class _Identifiers(object):
    ...
    def __init__(self, compiler, node=None, parent=None, nested=False):
        if parent is not None:
            ...
            if isinstance(node, nodes.NamespaceTag):
                self.declared = set()
    ...
    def visitBlockTag(self, node):
        if node is not self.node and not node.is_anonymous:
            ...
            if isinstance(self.node, nodes.DefTag):
                raise errors.CompileException(
                    "Named block '%s' not allowed inside of def '%s'"
                    % (node.name, self.node.name), **node.exception_kwargs)
            elif isinstance(self.node,
                            (nodes.CallTag, nodes.CallNamespaceTag)):
                raise errors.CompileException(
...
def mangle_choco_loop(node, printer):
