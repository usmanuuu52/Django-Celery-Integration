...
from choco import lexer
from choco import nodes
    ...
    def extract_nodes(self, nodes):
        ...
        for node in nodes:
            ...
            if in_translator_comments and \
                    isinstance(node, nodes.Text) and \
            ...
            if isinstance(node, nodes.DefTag):
                code = node.function_decl.code
                child_nodes = node.nodes
            elif isinstance(node, nodes.BlockTag):
                code = node.body_decl.code
                child_nodes = node.nodes
            elif isinstance(node, nodes.CallTag):
                code = node.code.code
                child_nodes = node.nodes
            elif isinstance(node, nodes.PageTag):
                code = node.body_decl.code
            elif isinstance(node, nodes.CallNamespaceTag):
                ...
                if node.isend:
                    ...
                    continue
                code = node.text
            elif isinstance(node, nodes.Code):
                in_translator_comments = False
                code = node.code.code
            elif isinstance(node, nodes.Expression):
                code = node.code.code
            else:
                continue
