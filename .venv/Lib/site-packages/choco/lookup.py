...
class TemplateCollection(object):
    ...
    def get_template(self, uri, relativeto=None, ui=False):
        ...
        :param relativeto: if present, the given ``uri`` is assumed to
         ...
         :param ui: if True, the template is a ui template.
...
class TemplateLookup(TemplateCollection):
    ...
    def __init__(self,
        ...
        if self.ui_container:
            self.ui_container.set_lookup(self)
    ...
    def load(self, filename, uri):
        ...
        try:
            ...
            try:
                ...
                self.collection[uri] = template = Template(
                    uri=uri,
                    filename=posixpath.normpath(filename),
                    lookup=self,
    ...
    def check(self, uri, template):
        ...
        try:
            ...
            if template.module._modified_time < \
                ...
                self.collection.pop(uri, None)
                return self.load(template.filename, uri)
            else:
                return template
        except OSError:
            self.collection.pop(uri, None)
