...
import traceback
import sys
from choco import util, compat


class ChocoException(Exception):
    pass


class RuntimeException(ChocoException):
    pass
...
class CompileException(ChocoException):

    def __init__(self, message, source, lineno, pos, filename):
        ChocoException.__init__(
            self,
...
class SyntaxException(ChocoException):
...
class UnsupportedError(ChocoException):

    """raised when a retired feature is used."""


class NameConflictError(ChocoException):

    """raised when a reserved word is used inappropriately"""


class TemplateLookupException(ChocoException):
    pass
...
class RichTraceback(object):
    ...
    Choco-specific template information.

    See the usage examples in :ref:`handling_errors`.
    ...
    def _init(self, trcback):
        ...
        for filename, lineno, function, line in rawrecords:
            ...
            try:
                ...
                try:
                    info = choco.template._get_module_info(filename)
                ...
                source_map = choco.template.ModuleInfo.\
...
def text_error_template(lookup=None):
    ...
    """
    import choco.template
    return choco.template.Template(r"""
<%page args="error=None, traceback=None"/>
<%!
    from choco.errors import RichTraceback
%>\
<%
...
def _install_pygments():
    ...
    from choco.ext.pygmentplugin import syntax_highlight  # noqa
    from choco.ext.pygmentplugin import pygments_html_formatter  # noqa


def _install_fallback():
    global syntax_highlight, pygments_html_formatter
    from choco.filters import html_escape
...
def html_error_template():
    ...
    return choco.template.Template(r"""
<%!
    from choco.errors import RichTraceback, syntax_highlight,\
...
<html>
<head>
    <title>Choco Runtime Error</title>
% endif
% if css:
    <style>
...
% for index in range(max(0, line-4),min(len(lines), line+5)):
    ...
    %>
        ${lines[index] | syntax_highlight(language='choco')}
    <%
       if pygments_html_formatter:
