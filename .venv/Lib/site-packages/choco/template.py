...
from choco import runtime, util, errors, codegen, cache, compat
import os
import re
import shutil
import stat
...
class Template(object):
    ...
    :param error_handler: Python callable which is called whenever
     compile or runtime errors occur. The callable is passed
    ...
    :param format_errors: if ``True``, errors which occur during
     the render phase of this template will be caught and
     formatted into an HTML error page, which then becomes the
     rendered result of the :meth:`.render` call. Otherwise,
     runtime errors are propagated outwards.
    ...
    def render_ui(self, context, *args, **kwargs):
        """Render this :class:`.Template` with the given context.

        The data is written to the context's buffer.

        """
        runtime._render_ui(self,
                                self.callable_,
                                context,
                                args,
                                kwargs)
...
class DefTemplate(Template):
    ...
    def __init__(self, parent, callable_):
        ...
        self.format_errors = parent.format_errors
